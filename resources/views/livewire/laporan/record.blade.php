<div class="mt-8">
    ...
    <div class="flex justify-between">
        ...
        <div>
            <button class="btn btn-sm hover:border-[#4F46E5] hover:text-[#4F46E5] hover: transition duration:200"
                type="submit">
    ...
    <div class="relative shadow-md sm:rounded-lg mt-2 border p-2" x-data="{ openModalDelete: false }" x-cloak>
            ...
            <div class="flex gap-1 items-center">
                ...
                <div class="relative text-center" x-data="{
                    ...
                    <div class="border p-2 w-16 cursor-pointer rounded-lg"
                        :class="isSelectOpen ? 'border-gray-600' : 'border-gray-400'"
                    ...
                    <tbody>
                        @foreach ($records as $record)
                            ...
                            <tr>
                                ...
                                <td><a href="{{ asset('storage/' . $record->file) }}"
                                        class="underline text-blue-500">File</a></td>
                                @hasanyrole('adminmaster|' . $kategori)
                                            ...
                                            <div class="relative">
                                                <label class="inline-flex items-center cursor-pointer"
                                                    ...
                                                    <input type="checkbox" value="" class="sr-only peer"
                                                        @checked(isset($record->published_at))
                                                        wire:change.prevent="publishOrDraft({{ $record->id }})"
                                                        @hasanyrole('adminmaster') disabled @endhasanyrole>
                                                    <div
                                        ...
                                        @if (isset($record->deleted_at))
                                            @hasanyrole($kategori)
                                                <span class="cursor-pointer p-2 hover:text-indigo-600"
                                                    wire:click.prevent="undo({{ $record->id }})"
                                                    id="undo{{ $record->id }}">
        ...
        <x-modal-alpine modalName="openModalDelete" title="Peringatan!">
            <div class="p-4 py-6 text-center">
                ...
                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-600">Apakah anda yakin ingin menghapus
                    data ini?</h3>
