...
class Record extends Component
{
    ...
    {
        $record = GelarDepan::query()->withTrashed()->find($id);
        $type = 'publik';
    ...
    {
        $this->totalAll = GelarDepan::query()->count();
        $this->totalPublik = GelarDepan::query()->published()->count();
        $this->totalKonsep = GelarDepan::query()->draft()->count();
        $this->totalTempatSampah = GelarDepan::query()->withTrashed()->whereNotNull('deleted_at')->count();
        $query = GelarDepan::query()
            ->when(strlen($this->search) > 2, function ($query) {
                $query
                    ->where(function ($query) {
                        $query
                            ->where('gelar_depan', 'like', '%' . $this->search . '%')
        ...
        return view('livewire.admin.data-master.gelar-depan.record', ['records' => $records]);
