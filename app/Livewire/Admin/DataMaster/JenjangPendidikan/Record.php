...
class Record extends Component
{
    ...
    {
        $record = JenjangPendidikan::query()->withTrashed()->find($id);
        $type = 'publik';
    ...
    {
        $record = JenjangPendidikan::query()->withTrashed()->whereNotNull('deleted_at')->find($id);
        $record->deleted_at = null;
        $record->save();
    ...
    public function delete($id): void
    {
        ...
        $record = JenjangPendidikan::query()->find($id);
        $record->published_at = null;
        $record->save();
        $record->delete();
    ...
    {
        $this->totalAll = JenjangPendidikan::query()->count();
        $this->totalPublik = JenjangPendidikan::query()->published()->count();
        $this->totalKonsep = JenjangPendidikan::query()->draft()->count();
        $this->totalTempatSampah = JenjangPendidikan::query()->withTrashed()->whereNotNull('deleted_at')->count();
        $query = JenjangPendidikan::query()
            ->when(strlen($this->search) > 2, function ($query) {
                $query
                    ->where(function ($query) {
                        $query
                            ->where('jenjang_pendidikan', 'like', '%' . $this->search . '%')
        ...
        return view('livewire.admin.data-master.jenjang-pendidikan.record', ['records' => $records]);
