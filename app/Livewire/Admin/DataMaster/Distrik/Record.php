...
class Record extends Component
{
    ...
    {
        $record = Distrik::query()->withTrashed()->find($id);
        $type = 'publik';
    ...
    {
        $record = Distrik::query()->withTrashed()->whereNotNull('deleted_at')->find($id);
        $record->deleted_at = null;
        $record->save();
    ...
    {
        $this->totalAll = Distrik::query()->count();
        $this->totalPublik = Distrik::query()->published()->count();
        $this->totalKonsep = Distrik::query()->draft()->count();
        $this->totalTempatSampah = Distrik::query()->withTrashed()->whereNotNull('deleted_at')->count();
        $query = Distrik::query()
            ->when(strlen($this->search) > 2, function ($query) {
                $query
                    ->where(function ($query) {
                        $query
                            ->where('distrik', 'like', '%' . $this->search . '%')
        ...
        return view('livewire.admin.data-master.distrik.record', ['records' => $records]);
