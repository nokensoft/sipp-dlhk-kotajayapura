...
class Record extends Component
{
    ...
    {
        $record = Suku::query()->withTrashed()->find($id);
        $type = 'publik';
    ...
    {
        $this->totalAll = Suku::query()->count();
        $this->totalPublik = Suku::query()->published()->count();
        $this->totalKonsep = Suku::query()->draft()->count();
        $this->totalTempatSampah = Suku::query()->withTrashed()->whereNotNull('deleted_at')->count();
        $query = Suku::query()
            ->when(strlen($this->search) > 2, function ($query) {
                $query
                    ->where(function ($query) {
                        $query
                            ->where('suku', 'like', '%' . $this->search . '%')
        ...
        return view('livewire.admin.data-master.suku.record', ['records' => $records]);
