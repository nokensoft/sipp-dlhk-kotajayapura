...
class Record extends Component
{
    ...
    {
        $record = GelarBelakang::query()->withTrashed()->find($id);
        $type = 'publik';
    ...
    {
        $this->totalAll = GelarBelakang::query()->count();
        $this->totalPublik = GelarBelakang::query()->published()->count();
        $this->totalKonsep = GelarBelakang::query()->draft()->count();
        $this->totalTempatSampah = GelarBelakang::query()->withTrashed()->whereNotNull('deleted_at')->count();
        $query = GelarBelakang::query()
            ->when(strlen($this->search) > 2, function ($query) {
                $query
                    ->where(function ($query) {
                        $query
                            ->where('gelar_belakang', 'like', '%' . $this->search . '%')
        ...
        return view('livewire.admin.data-master.gelar-belakang.record', ['records' => $records]);
