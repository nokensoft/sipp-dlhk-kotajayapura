...
class Record extends Component
{
    ...
    {
        $record = Agama::query()->withTrashed()->find($id);
        $type = 'publik';
    ...
    {
        $this->totalAll = Agama::query()->count();
        $this->totalPublik = Agama::query()->published()->count();
        $this->totalKonsep = Agama::query()->draft()->count();
        $this->totalTempatSampah = Agama::query()->withTrashed()->whereNotNull('deleted_at')->count();
        $query = Agama::query()
            ->when(strlen($this->search) > 2, function ($query) {
                $query
                    ->where(function ($query) {
                        $query
                            ->where('agama', 'like', '%' . $this->search . '%')
        ...
        return view('livewire.admin.data-master.agama.record', ['records' => $records]);
