...
class Record extends Component
{
    ...
    {
        $record = StatusPerkawinan::query()->withTrashed()->find($id);
        $type = 'publik';
    ...
    {
        $this->totalAll = StatusPerkawinan::query()->count();
        $this->totalPublik = StatusPerkawinan::query()->published()->count();
        $this->totalKonsep = StatusPerkawinan::query()->draft()->count();
        $this->totalTempatSampah = StatusPerkawinan::query()->withTrashed()->whereNotNull('deleted_at')->count();
        $query = StatusPerkawinan::query()
            ->when(strlen($this->search) > 2, function ($query) {
                $query
                    ->where(function ($query) {
                        $query
                            ->where('status_perkawinan', 'like', '%' . $this->search . '%')
        ...
        return view('livewire.admin.data-master.status-perkawinan.record', ['records' => $records]);
