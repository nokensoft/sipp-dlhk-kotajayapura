...
class Record extends Component
{
    ...
    {
        $record = Kelurahan::query()->withTrashed()->find($id);
        $type = 'publik';
    ...
    {
        $record = Kelurahan::query()->withTrashed()->whereNotNull('deleted_at')->find($id);
        $record->deleted_at = null;
        $record->save();
    ...
    public function delete($id): void
    {
        ...
        $record = Kelurahan::query()->find($id);
        $record->published_at = null;
        $record->save();
        $record->delete();
    ...
    {
        $this->totalAll = Kelurahan::query()->count();
        $this->totalPublik = Kelurahan::query()->published()->count();
        $this->totalKonsep = Kelurahan::query()->draft()->count();
        $this->totalTempatSampah = Kelurahan::query()->withTrashed()->whereNotNull('deleted_at')->count();
        $query = Kelurahan::query()
            ->when(strlen($this->search) > 2, function ($query) {
                $query
                    ->where(function ($query) {
                        $query
                            ->where('kelurahan', 'like', '%' . $this->search . '%')
        ...
        return view('livewire.admin.data-master.kelurahan.record', ['records' => $records]);
