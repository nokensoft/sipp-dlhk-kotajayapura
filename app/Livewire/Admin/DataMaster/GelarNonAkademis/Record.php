...
class Record extends Component
{
    ...
    {
        $record = GelarNonAkademis::query()->withTrashed()->find($id);
        $type = 'publik';
    ...
    {
        $this->totalAll = GelarNonAkademis::query()->count();
        $this->totalPublik = GelarNonAkademis::query()->published()->count();
        $this->totalKonsep = GelarNonAkademis::query()->draft()->count();
        $this->totalTempatSampah = GelarNonAkademis::query()->withTrashed()->whereNotNull('deleted_at')->count();
        $query = GelarNonAkademis::query()
            ->when(strlen($this->search) > 2, function ($query) {
                $query
                    ->where(function ($query) {
                        $query
                            ->where('gelar_non_akademis', 'like', '%' . $this->search . '%')
        ...
        return view('livewire.admin.data-master.gelar-non-akademis.record', ['records' => $records]);
